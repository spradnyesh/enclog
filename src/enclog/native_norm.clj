...
(extend-protocol Normalisable
  ...
  ;; if this fires, we're dealing with a Java list-like collection - return a vector
  java.util.List
  (normalise
    ([this transform]
  ...
  ;; if this fires, we don't know the type so  we'll return a lazy-seq
  ...
  ;; sets are typically not ordered so ordering will dissapear after processing
  ...
  ;; assuming a map with collections for keys AND values (a dataset perhaps?)
